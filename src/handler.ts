...
import { Deployment, ListDeploymentsRequest, ListDeploymentsResponse, Ref, Status } from 'waypoint-node-pb';
...
async function updateDeployStatusForRun(ctx: Ctx, workspace: string): Promise<boolean> {
  ...
  const deployments = await new Promise<Deployment[]>((resolve, reject) => {
    ctx.waypoint.listDeployments(req, (err, resp: ListDeploymentsResponse) => {
      if (err) {
...
export async function handlePush(ctx: Ctx, payload: EventPayloads.WebhookPayloadPush): Promise<void> {
  ...
  const waypointOptions = [
    ...
    '-label',
    `${LABEL_PREFIX}/vcs-url/${commit.data.url}`,
    '-label',
    `${LABEL_PREFIX}/vcs-run-id/${ctx.context.runId}`,
  ];

  core.group('waypoint build', async () => {
    // Run the build
  ...
  core.group('waypoint deploy', async () => {
